...
exports.nextChallanNumber = nextChallanNumber;
...
const dbDir = path_1.default.dirname(dbFile);
...
function initSchema() {
  ...
  `);
    const c = exports.db.prepare("SELECT value FROM counters WHERE name = 'challan'").get();
    if (!c)
        exports.db.prepare("INSERT INTO counters (name, value) VALUES ('challan', 0)").run();
...
function seedWarehousesAndCities() {
    const existing = exports.db.prepare("SELECT COUNT(*) as c FROM warehouses").get();
    if (existing.c === 0) {
        const insert = exports.db.prepare(`INSERT INTO warehouses (code, name, city, lat, lon, capacity) VALUES (?, ?, ?, ?, ?, ?)`);
        for (const w of DEFAULT_WAREHOUSES)
            insert.run(w.code, w.name, w.city, w.lat, w.lon, w.capacity);
    }
    const insertCity = exports.db.prepare(`INSERT OR IGNORE INTO city_coords (city, lat, lon) VALUES (?, ?, ?)`);
    for (const c2 of DEFAULT_CITY_COORDS)
        insertCity.run(c2.city, c2.lat, c2.lon);
}
function nextChallanNumber() {
    const row = exports.db.prepare("UPDATE counters SET value = value + 1 WHERE name = 'challan' RETURNING value").get();
