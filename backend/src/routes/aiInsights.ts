...
import { generateInsight, getLatestInsight, listInsights, applyDecision } from "../services/aiInsightsService";
...
router.post("/:productId/generate", (req, res, next) => {
  try {
    const insight = generateInsight(Number(req.params.productId));
...
router.get("/:productId/latest", (req, res, next) => {
  try {
    const insight = getLatestInsight(Number(req.params.productId));
    if (!insight) return res.status(404).json({ error: "No recommendation generated yet" });
...
router.get("/:productId/history", (req, res, next) => {
  try {
    const rows = listInsights(Number(req.params.productId)).map((i) => ({ ...i, justification: JSON.parse(i.justification) }));
    res.json(rows);
  } catch (err) {
    next(err);
  }
...
router.post("/:id/decision", (req, res, next) => {
  try {
    ...
    const insight = applyDecision(Number(req.params.id), decision, qty ? Number(qty) : undefined, String(decidedBy));
    res.json({ ...insight, justification: JSON.parse(insight.justification) });
  } catch (err) {
    next(err);
  }
