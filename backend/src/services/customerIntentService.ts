...
export function cityForIndex(i: number): string {
 ...
 */
export function findOrCreateCustomer(name: string, city?: string): number {
  const existing = db.prepare(`SELECT id FROM customers WHERE name = ? COLLATE NOCASE`).get(name) as
    | { id: number }
    | undefined;
  ...
  const info = db
    .prepare(
      `INSERT INTO customers (name, mobile, email, customer_type, address, status)
 ...
 */
export function computeIntent(customerId: number): IntentResult {
  const row = db
    .prepare(
      `SELECT
